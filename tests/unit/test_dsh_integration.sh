...
code=$?
assert_eq "0" "$code" "a dirty workspace remains supported"
run_dir="$(find "$AGENT_STATE_HOME/runs" -path '*dirty-baseline*' -name run.json -printf '%h\n' | sort | tail -1)"
assert_contains "$(cat "$run_dir/diff.txt")" "add.js" "review diff includes the lead implementation change"
assert_not_contains "$(cat "$run_dir/diff.txt")" "NOTES.md" "review diff excludes the user's pre-existing dirty file"

make_node_fixture "$TMP/ambient-code"
export DSH_TOOLS_MODE=code
: > "$MOCK_DSH_CALL_LOG"
out="$(run_agent --engine dsh --tool-mode native "$TMP/ambient-code" "Add validation to prevent negative numbers." 2>&1)"
code=$?
assert_eq "0" "$code" "ambient code mode cannot break native execution"
assert_contains "$(cat "$MOCK_DSH_CALL_LOG")" $'tools_mode=native' "ambient code mode is overwritten before every DSH call"

make_node_fixture "$TMP/ambient-both"
export DSH_TOOLS_MODE=both
: > "$MOCK_DSH_CALL_LOG"
out="$(run_agent --engine dsh --tool-mode native "$TMP/ambient-both" "Add validation to prevent negative numbers." 2>&1)"
code=$?
assert_eq "0" "$code" "ambient both mode cannot break native execution"
